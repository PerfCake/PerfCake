...
public class ScenarioExecution {
   ...
   public static void execute(final String scenarioFile, final Properties properties) throws PerfCakeException {
      ...
      properties.forEach((k, v) -> {
         ...
         log.debug(String.format("Setting property %s=%s", k.toString(), v.toString()));
      });

      Utils.initTimeStamps();
      log.debug(String.format("Debug Agent pre: %s", PerfCakeDebug.getInstance()));
      Utils.initDebugAgent();
      log.debug(String.format("Debug Agent post: %s", PerfCakeDebug.getInstance()));
